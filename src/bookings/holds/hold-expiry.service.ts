...
import type { Tx } from '../strategies/reserve.strategy';
...
export class HoldExpiryService {
  ...
  async expire(bookingId: string): Promise<'expired' | 'skipped'> {
    const rows = await retryOnConflict(() =>
      this.prisma.$transaction(async (tx) => {
        ...
        await this.announce(tx, expired);
        return expired;
      }),
    );
  ...
  private announce(tx: Tx, rows: ExpiredRow[]): Promise<unknown> {
    return this.outbox.enqueueMany(
      tx,
      rows.map((row) => ({
        type: 'booking.expired' as const,
        payload: { bookingId: row.id, paymentId: row.paymentId },
      })),
    );
  }
